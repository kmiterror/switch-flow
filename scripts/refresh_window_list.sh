...
LOG_FILE="$HOME/Library/Logs/SwitchFlow/refresh_window_list.log"
...
refresh_cache() {
  ...
  if yabai -m query --windows id,title,app >"$temp_file"; then
    ...
    echo "Cache refreshed successfully in ${elapsed_seconds}s at $(date)" >> "$LOG_FILE"

  else
    echo "Failed to refresh cache at $(date)" >> "$LOG_FILE"
    rm -f "$temp_file"
  fi
